...
public class Toolbox {
  ...
  public static Map<Integer, Integer> countOccurrences(SingleNode head) {
    ...
    SingleNode current = head;
    while(current != null) { 
      if(occurs.containsKey(current.data) == false) { 
        occurs.put(current.data, 1); 
      } else { 
        int temp = occurs.get(current.data) + 1;
        occurs.replace(current.data, temp);
      }
      current = current.next; 
    }
    return occurs; 
  ...
  public static void removeNode(DoubleNode node) {
    ...
    DoubleNode thePrev = node.prev;
    DoubleNode theNext = node.next;
    if (thePrev == null && theNext == null) { node = null; }
    else if (thePrev == null) { theNext.prev = null; }
    else if (theNext == null) { thePrev.next = null; }
    else {
  ...
  public static void removeGiants(SingleNode head) {
    ...
    while(current.next != null) {
      if(current.next.data < current.data) { behind.next = current.next; }
      behind = current;
      current = current.next;
    }
  }
  ...
  public static boolean hasBalancedParentheses(String input) {
    ...
    {
      if(charInput[i] == '(') { storage.push(charInput[i]); }
      else {
        if(storage.empty() == false) { storage.pop(); }
        else { return false; }
      }
    }
